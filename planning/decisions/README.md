# Architecture decision records

ADRs preserve decisions that alter scope, architecture, tooling, security, or
evidence. Do not edit a completed ADR to hide a later change; add a new ADR that
supersedes it and link both records.

- [`ADR-0001-v1-local-first.md`](ADR-0001-v1-local-first.md)
- [`ADR-0002-repository-topology.md`](ADR-0002-repository-topology.md)
